...
ripple_mat, features, labels =load_ripple_data(dataset)
N=ripple_mat.shape[0]
# print(adj.shape)
# print(features.shape)
# print(labels.shape)
idx_train = range((N//5)*5)
idx_val = range(N//3,(N//3)*2)
idx_test=idx_val
...
def train(epoch):
    ...
    output = model(features, ripple_mat)
...
def save_embeddings(dataset='cora'):
    model.eval()
    output = model.savector(features, ripple_mat)
    outVec = output.cpu().detach().numpy()
    path = Path(__file__).parent / 'Ripple_result'/('{}_outVec.txt'.format(dataset))
    #这里第一个位置为地址，第二个位置为内容
    np.savetxt(path, outVec)
    path = Path(__file__).parent / 'Ripple_result'/('{}_labels.txt'.format(dataset))
